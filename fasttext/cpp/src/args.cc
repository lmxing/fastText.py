...
void Args::printHelp() {
  ...
  std::cout
    ...
    << "  -input              training file path 训练集路径\n"
    << "  -output             output file path 测试集路径\n\n"
    << "The following arguments are optional:\n"
    << "  -lr                 learning rate 学习率[" << lr << "]\n"
    << "  -lrUpdateRate       change the rate of updates for the learning rate 学习率更新速率[" << lrUpdateRate << "]\n"
    << "  -dim                size of word vectors 词向量的维度[" << dim << "]\n"
    << "  -ws                 size of the context window 滑动窗口维度[" << ws << "]\n"
    << "  -epoch              number of epochs 长度[" << epoch << "]\n"
    << "  -minCount           minimal number of word occurences 词最小次数 [" << minCount << "]\n"
    << "  -minCountLabel      minimal number of label occurences 最小标签[" << minCountLabel << "]\n"
    << "  -neg                number of negatives sampled 负样本个数[" << neg << "]\n"
    << "  -wordNgrams         max length of word ngram ngram的最大n [" << wordNgrams << "]\n"
    << "  -loss               loss function {ns, hs, softmax} [ns] 损失函数：\n"
    << "  -bucket             number of buckets 桶的个数 [" << bucket << "]\n"
    << "  -minn               min length of char ngram  ngram的最小长度[" << minn << "]\n"
    << "  -maxn               max length of char ngram ngram的最大长度[" << maxn << "]\n"
    << "  -thread             number of threads 线程个数[" << thread << "]\n"
    << "  -t                  sampling threshold 采样阈值[" << t << "]\n"
    << "  -label              labels prefix  先验label[" << label << "]\n"
